...
import '../config.dart'; // Still needed for potential future use
    ...
    Future<void> _askGemini() async {
      ...
      try {
        ...
        if (user == null) {
          ...
          setState(() {
            _aiResponse = 'Please sign in to use Gemini.';
          });
          Navigator.push(context, MaterialPageRoute(builder: (_) => LoginScreen()));
          return;
        }

        // Check for Vertex AI API key (for logging purposes)
        final apiKey = Config.vertexAiApiKey;
        if (apiKey.isEmpty) {
          print('Vertex AI API key not found in .env');
        } else {
          print('Vertex AI API key loaded: $apiKey');
        }
        ...
        setState(() {
          _aiResponse = 'Error: Failed to get a response from the AI assistant. $e';
        });
      } finally {
        setState(() {
          _isAiLoading = false;
      ...
      body: SingleChildScrollView( // Add SingleChildScrollView
        child: Column(
          children: [
            ...
            SizedBox(
              height: 200, // Constrain the height of the chat list
